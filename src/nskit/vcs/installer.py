...
class Installer(ABC, BaseConfiguration):
    ...
    def check_repo(self, path: Path, **kwargs):
        ...
        raise NotImplementedError(
            "Implement in a language specific installer. It should check for appropriate files to signal that it is a repo of that type"
        )

    @abstractmethod
    def install(self, path: Path, *, deps: bool = True, **kwargs):
        """Install the repo into the appropriate environment."""
        raise NotImplementedError(
            "Implement in language specific installer. It should take in any language specific environment/executables"
        )
