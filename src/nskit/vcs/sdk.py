...
class SDK(BaseConfiguration):
    root_dir: Path = Field(default_factory=Path.cwd)
    settings: Annotated[SDKSettings, Field(validate_default=True)] = None
    ...
    def list_repos(self):
        ...
        potential_repos = self._client.list()
        sdk_repos = []
    ...
    def virtualenv(self):
        ...
        if not self._full_virtualenv_dir.exists():
            virtualenv.cli_run([str(self._full_virtualenv_dir), '--seeder', 'azdo-pip'])
