...
class FileConfigSettingsSource(PydanticBaseSettingsSource):
    ...
    def get_field_value(
        ...
        if self.__parsed_contents is None:
            try:
                ...
                if file_path.suffix.lower() in ['.jsn', '.json'] or file_type.lower() == 'json':
                    self.__parsed_contents = json.loads(file_contents)
                elif file_path.suffix.lower() in ['.tml', '.toml'] or file_type.lower() == 'toml':
                    self.__parsed_contents = toml.loads(file_contents)
                elif file_path.suffix.lower() in ['.yml', '.yaml'] or file_type.lower() == 'yaml':
    ...
    def _load_file(self, file_path: Path, encoding: str) -> Dict[str, Any]:
