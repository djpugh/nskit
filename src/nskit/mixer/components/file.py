...
from ..utilities import JINJA_ENVIRONMENT, Resource


class File(FileSystemObject):

    content: Union[Resource, str, bytes, Path] = Field('', description='The file content')
    ...
    def dryrun(self, base_path: Path, context: Dict[str, Any], override_path: Optional[Path] = None):
        ...
        file_path = self.get_path(base_path, context, override_path)
        result = {file_path: self.render_content(context)}
        return result
