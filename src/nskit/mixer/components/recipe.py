...
class Recipe(Folder):
    ...
    def create(self, base_path: Optional[Path] = None, override_path: Optional[Path]=None, **additional_context):
        ...
        for hook in self.pre_hooks:
            recipe_name, recipe_path, context = hook(recipe_name, recipe_path, context)
        folder_path = self.write(recipe_path.parents, context, override_path=recipe_name)
        recipe_name = folder_path.parts[-1]
    ...
    def dryrun(
        ...
        if base_path is None:
            base_path = Path.cwd()
        return super().dryrun(base_path=base_path, context=context, override_path=override_path)
    ...
    def validate(
        ...
        if base_path is None:
            base_path = Path.cwd()
        return super().validate(base_path=base_path, context=context, override_path=override_path)
