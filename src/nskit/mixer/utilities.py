...
class _PkgResourcesTemplateLoader(BaseLoader):
    ...
    def get_source(environment, template):  # noqa: U100
        ...
        try:
            ...
            raise TemplateNotFound(template, *e.args) from None
...
class _EnvironmentFactory:
    ...
    def add_extensions(self, environment: Environment):
        ...
        NskitMixerExtensionOptions = ExtensionsEnum.from_entrypoint(
            "NskitMixerExtensionOptions", "nskit.mixer.environment.extensions"
        )
    ...
    def get_environment(self) -> Environment:
        ...
        NskitMixerEnvironmentOptions = ExtensionsEnum.from_entrypoint(
            "NskitMixerEnvironmentOptions", "nskit.mixer.environment.factory"
        )
        if sys.version_info.major <= 3 and sys.version_info.minor < 12:
            if selected_method not in NskitMixerEnvironmentOptions.__members__.keys():
                raise ValueError(
                    f"NSKIT_MIXER_ENVIRONMENT_FACTORY value {selected_method} not installed - available options are {list(NskitMixerEnvironmentOptions)}"
                )
        else:
