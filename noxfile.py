...
def install_dependencies(session, required=True, optional=None, pyproject_toml="pyproject.toml"):
    if optional is None:
        optional = []
...
@nox.session(reuse_venv=True, tags=["format"])
def format(session):
    install_dependencies(session, required=False, optional=["dev", "dev-lint"])
...
def licenses(session):
    ...
    session.run(
        "licensecheck",
        "--using",
        "PEP631:github;azure_devops;dev;dev-test;dev-lint;dev-types;dev-security;dev-docs;dev-build",
        "--format",
        "json",
        "--file",
        "licenses-dev.json",
