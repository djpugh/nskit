...
from nskit.vcs.namespace_validator import _DELIMITERS, REPO_SEPARATOR, NamespaceOptionsType, NamespaceValidator
...
class NamespaceOptionsTypeTestCase(unittest.TestCase):
    ...
    def test_complex(self):
        self.assertEqual(
            self.ta.validate_python([{"a": [{"b": ["c", "d"]}, "e"]}, "f"]), [{"a": [{"b": ["c", "d"]}, "e"]}, "f"]
        )

    def test_wrong_types(self):
        with self.assertRaises(ValidationError):
            self.ta.validate_python([{"a": [{"b": ["c", "d", 3]}, "e"]}, "f"])
        with self.assertRaises(ValidationError):
            self.ta.validate_python({"a": [{"b": ["c", "d"]}, "e"]})

        with self.assertRaises(ValidationError):
            self.ta.validate_python([{"a": [{"b": ["c", ["d"]]}, "e"]}, "f"])


class NamespaceValidatorTestCase(unittest.TestCase):
    def setUp(self):
        self.ns_options = [{"a": [{"b": ["c", "d"]}, "e"]}, "f"]
    ...
    def test_init_wrong_options(self):
        with self.assertRaises(ValidationError):
            NamespaceValidator(options=[{"a": [{"b": ["c", "d", 3]}, "e"]}, "f"])
        with self.assertRaises(ValidationError):
            NamespaceValidator(options={"a": [{"b": ["c", "d"]}, "e"]})

        with self.assertRaises(ValidationError):
            NamespaceValidator(options=[{"a": [{"b": ["c", ["d"]]}, "e"]}, "f"])
    ...
    def test_validate_name_wrong(self):
        nsv = NamespaceValidator(options=self.ns_options)
        self.assertEqual(
            nsv.validate_name("a.b.g"),
            (False, "Does not match valid names for b: c, d, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            nsv.validate_name("a.b.g.x"),
            (False, "Does not match valid names for b: c, d, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            nsv.validate_name("a.b.f"),
            (False, "Does not match valid names for b: c, d, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            nsv.validate_name("a.b.f.z"),
            (False, "Does not match valid names for b: c, d, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            nsv.validate_name("a.c"),
            (False, "Does not match valid names for a: b, e, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            nsv.validate_name("a.c.x"),
            (False, "Does not match valid names for a: b, e, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            nsv.validate_name("g"),
            (False, "Does not match valid names for <root>: a, f, with delimiters: ['.', ',', '-']"),
        )
    ...
    def test_validate_name_delimiters_wrong(self):
        ...
        name = "a-b-c"
        self.assertEqual(
            nsv.validate_name(name), (False, "Does not match valid names for <root>: a, f, with delimiters: ['.', '/']")
        )
