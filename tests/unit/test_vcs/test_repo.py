...
from unittest.mock import MagicMock, NonCallableMagicMock, call, create_autospec, patch
...
from nskit.vcs import installer, repo
from nskit.vcs.namespace_validator import _DELIMITERS, REPO_SEPARATOR, NamespaceValidator
from nskit.vcs.providers.abstract import RepoClient, VCSProviderSettings
from nskit.vcs.repo import NamespaceValidationRepo, Repo, ValidationEnum, _Repo
    ...
    def test_commit_paths_no_hooks(self, sp):
        ...
        with ChDir():
            ...
            sp.check_call.assert_has_calls(
                [call(["git", "add", "a*"]), call(["git", "commit", "--no-verify", "-m", "x"])]
            )
...
class NamespaceValidationRepoTestCase(unittest.TestCase):
    ...
    def test_init_storage(self):
        r = NamespaceValidationRepo(
            namespaces_filename="test.yaml", name="abc", provider_client=RepoClient(), local_dir="abc/def"
        )
    ...
    def test_validate_name(self):
        ...
        self.assertEqual(
            r.validate_name("x"),
            (False, "Does not match valid names for <root>: a, d, with delimiters: ['.', ',', '-']"),
        )
        self.assertEqual(
            r.validate_name("a.x"), (False, "Does not match valid names for a: b, c, with delimiters: ['.', ',', '-']")
        )
    ...
    def test_load_namespace_validator_local_exists_custom_name(self):
        with ChDir():
            ...
            cl.get_remote_url.return_value = "https://test.com"
            r = NamespaceValidationRepo(
                namespaces_filename="namespaces2.yaml", name="abc", provider_client=cl, local_dir=Path("test/abc")
            )
            r._git_repo_cls = self._MockedGitRepoKls
            r._git_repo.remotes.origin = create_autospec(git.Remote)
            nsv2 = r._load_namespace_validator()
    ...
    def test_load_namespace_validator_local_exists_temp(self):
        with ChDir():
            ...
            cl = self._MockedRepoClientKls()
            cl.get_remote_url.return_value = "https://test.com"
            r = NamespaceValidationRepo(namespaces_filename="namespaces2.yaml", name="abc", provider_client=cl)
            r._git_repo_cls = self._MockedGitRepoKls
            r._git_repo.remotes.origin = create_autospec(git.Remote)
            nsv2 = r._load_namespace_validator()
            r._git_repo.remotes.origin.pull.assert_called_once_with()
    ...
    def test_create_from_validator(self, sp):
        with ChDir():
            ...
            cl.check_exists.return_value = False
            r = NamespaceValidationRepo(
                namespaces_filename="namespaces2.yaml", name="abc", provider_client=cl, local_dir=Path.cwd()
            )
            r._git_repo_cls = self._MockedGitRepoKls
    ...
    def test_create_from_validator_override(self, sp):
        with ChDir():
            ...
            cl.get_remote_url.return_value = None
            cl.check_exists.return_value = False
            r = NamespaceValidationRepo(name="abc", provider_client=cl, local_dir=Path.cwd())
            r._git_repo_cls = self._MockedGitRepoKls
            r._git_repo.working_tree_dir = Path.cwd()
...
class RepoTestCase(unittest.TestCase):
    ...
    def test_validate_name_validation_ok(self, sp):
        with ChDir():
            ...
            r.create(namespace_options=options, delimiters=[".", "-"], repo_separator="-")
            repo = Repo(
                name="a.b.x", provider_client=cl, namespace_validation_repo=r, validation_level=ValidationEnum.strict
            )
    ...
    def test_validate_name_validation_warn(self, sp):
        with ChDir():
            ...
            with self.assertWarns(Warning):
                Repo(
                    name="a.g.x", provider_client=cl, namespace_validation_repo=r, validation_level=ValidationEnum.warn
                )
            with self.assertWarns(Warning):
        ...
        with ChDir():
            ...
            with self.assertRaises(ValidationError):
                Repo(
                    ...
                    validation_level=ValidationEnum.strict,
                )
            with self.assertRaises(ValidationError):
                Repo(
                    name="abc", provider_client=cl, namespace_validation_repo=r, validation_level=ValidationEnum.strict
                )
