...
class TestCreateCLI:
    ...
    def test_create_cli_basic(self):
        ...
        app = create_cli(
            recipe_entrypoint="test.recipes",
            app_name="test-cli",
            app_help="Test CLI"
        )
    ...
    def test_list_command_without_backend(self, runner):
        """Test list command is not available without backend."""
        app = create_cli(recipe_entrypoint="test.recipes")
        result = runner.invoke(app, ["--help"])
    ...
    def test_init_command_without_backend(self, mock_load, runner, tmp_path):
        ...
        result = runner.invoke(app, [
            "init",
            "--recipe", "test_recipe",
            "--output-base-path", str(tmp_path)
        ])
        
        mock_load.assert_called_once_with("test_recipe", entrypoint="test.recipes")
    ...
    def test_get_required_fields_command(self, mock_load, runner):
        ...
        with patch('nskit.mixer.utilities.get_required_fields_as_dict', return_value={"field1": "str"}):
            result = runner.invoke(app, [
                "get-required-fields",
                "--recipe", "test_recipe"
            ])
