...
class RecipeTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self._recipe = Recipe(
            ...
            version="0.1.0",
            contents=[
                Folder(id_="a", name="folder", contents=[File(id_="b", name="test{{x.a}}.txt", content="test")]),
                Folder(id_="b", name="folder2", contents=[File(id_="b", name="test2.txt", content="test2{{x.a}}")]),
            ],
        )
    ...
    def test_recipe(self):
        self.assertEqual(
            self._recipe.recipe,
            {"name": "nskit.mixer.components.recipe:Recipe", "version": "0.1.0", "extension_name": "Recipe"},
        )
    ...
    def test_create_no_override(self):
        with ChDir():
            ...
            with open("test/.recipe-batch.yaml") as fp:
                batch = yaml.load(fp)
                expected_name = f"{self._complex_recipe.__class__.__module__}:TestRecipe"
                self.assertEqual(len(batch), 1)
                self.assertEqual(
                    batch[0]["context"],
                    {
                        "x": {"a": 1, "random_property": "abacus"},
                        "recipe": {"extension_name": "TestRecipe", "name": expected_name, "version": None},
                    },
                )
                self.assertEqual(batch[0]["nskit_version"], __version__)
                self.assertEqual(
                    batch[0]["recipe"], {"name": expected_name, "version": None, "extension_name": "TestRecipe"}
                )
    ...
    def test_context_with_properties(self):
        ...
        expected_name = f"{TestRecipe.__module__}:TestRecipe"
        self.assertEqual(
            t.context,
            {
                "random_property": "abacus",
                "recipe": {"extension_name": "TestRecipe", "name": expected_name, "version": None},
            },
        )
    ...
    def test_dryrun_no_override(self):
        self.assertEqual(
            ...
            {
                Path("test"): {
                    Path("test/folder"): {Path("test/folder/test1.txt"): "test"},
                    Path("test/folder2"): {Path("test/folder2/test2.txt"): "test21"},
                }
            },
        )
    ...
    def test_dryrun_override(self):
        self.assertEqual(
            ...
            {
                Path("abc"): {
                    Path("abc/folder"): {Path("abc/folder/test1.txt"): "test"},
                    Path("abc/folder2"): {Path("abc/folder2/test2.txt"): "test21"},
                }
            },
        )
    ...
    def test_validate_missing_contents(self):
        with ChDir():
            ...
            self.assertEqual(errors, [])
            self.assertEqual(
                set(ok), {Path("test").absolute() / "folder2", Path("test").absolute() / "folder2" / "test2.txt"}
            )
    ...
    def test_load_recipe_found(self):
        with TestExtension("test_recipe", "nskit.recipes", self._complex_recipe_cls):
            ...
            self.assertEqual(
                ...
                {
                    Path("abacus"): {
                        Path("abacus/folder"): {Path("abacus/folder/test3.txt"): "test"},
                        Path("abacus/folder2"): {Path("abacus/folder2/test2.txt"): "test23"},
                    }
                },
            )
    ...
    def test_repr(self):
        ...
        expected_module = self._complex_recipe.__class__.__module__
        self.assertEqual(
            out,
            f"test = TestRecipe(name: test):\n|- folder = Folder(id: a, name: folder):\n  |- test1.txt = File(id: b, name <TemplateStr>: test{{{{x.a}}}}.txt)\n|- folder2 = Folder(id: b, name: folder2):\n  |- test2.txt = File(id: b, name: test2.txt)\n\nContext: {{'x': {{'a': 1, 'random_property': 'abacus'}}, 'recipe': {{'name': '{expected_module}:TestRecipe', 'version': None, 'extension_name': 'TestRecipe'}}}}",
        )
